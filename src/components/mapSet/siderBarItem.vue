	...
	<div>
		<div v-for="(items,indexOne) in barMenu" :key="indexOne + '-one'">
			<el-submenu :index="items.name" v-if="items.children.length>0" class="el-menu-title-box">
				<div slot="title" @click="goRouter(items)">
					<i class="el-icon-share two-icon" @click.stop.prevent="hideLayer(items,items.show,indexOne)" v-show="items.show"></i>
					<i class="el-icon-delete two-icon" @click.stop.prevent="hideLayer(items,items.show,indexOne)" v-show="!items.show"></i>
				...
				<div v-for="(item,indexTwo) in items.children" :key="indexTwo + '-two'">
					<el-menu-item :index="item.name" v-if="item.children.length<=0" :class="{ active:item.active}">
						<i class="el-icon-share two-icon" @click.stop.prevent="hideLayer(item,item.show,indexOne,false)" v-show="item.show"></i>
						<i class="el-icon-delete two-icon" @click.stop.prevent="hideLayer(item,item.show,indexOne,false)" v-show="!item.show"></i>
						<span>{{item.name}}</span>
					</el-menu-item>
				</div>
			</el-submenu>
			<el-menu-item :index="items.name" class="submenu-title-noDropdown" :class="{ active:items.active}" v-if="items.children.length<=0">
				<i class="el-icon-share two-icon" @click.stop.prevent="hideLayer(items,items.show,indexOne,true)" v-show="items.show"></i>
				<i class="el-icon-delete two-icon" @click.stop.prevent="hideLayer(items,items.show,indexOne,true)" v-show="!items.show"></i>
				<span slot="title">{{items.name}}</span>
			</el-menu-item>
		</div>
		<el-menu-item index="newLayer" class="submenu-title-noDropdown" >
			<i class="el-icon-shopping-cart-full two-icon"></i>
			<span slot="title">我的图层</span>
			<i class="el-icon-plus delete-icon two-icon" @click.stop.prevent="openAddModal"></i>
		</el-menu-item>
		<div v-for="(obj,num) in layerList" class="my-layer" v-bind:key="num + '-num'">
			<el-menu-item :index="obj.name" class="submenu-title-noDropdown" :class="{ active:obj.active}" v-if="obj.children.length<=0" @click="goRouter(obj,true)">
				<i class="el-icon-share two-icon" @click.stop.prevent="hideCustomizeLayer(obj,obj.show)" v-show="obj.show"></i>
				<i class="el-icon-delete two-icon" @click.stop.prevent="hideCustomizeLayer(obj,obj.show)" v-show="!obj.show"></i>
				<i class="el-icon-delete two-icon"></i>
				<span slot="title">{{obj.name}}</span>
				<i class="el-icon-close delete-icon two-icon" @click.stop.prevent="deleteLayer(num)"></i>
			</el-menu-item>
		</div>
	</div>
</template>
